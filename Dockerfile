...
RUN cd /usr/local/blender/$BLENDER_MAJOR/python/bin \
    ...
    && ./python3.5m -m pip install trimesh natsort==7.0.1 networkx==2.1 scipy==1.1.0 opencv-python==3.1.0 scikit-image transforms3d plyfile meshlabxml
...
RUN cd /usr/local/blender/$BLENDER_MAJOR/python/bin \
    && ./python3.5m -m pip install pytransform3d
