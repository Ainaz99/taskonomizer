...
def sample_cubefaces(cube_faces, tp, coor_y, coor_x, order):
    ...
    return map_coordinates(cube_faces, [tp, coor_y, coor_x], order=order, mode='nearest')
...
def e2c(e_img, face_w=1024, mode='nearest', cube_format='list'):
    '''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, help="Name of taskonomy model")
    parser.add_argument("--sceneGraph_data_path", type=str, help="Path to 3D Scene Graph data")
    parser.add_argument("--taskonomy_data_path", type=str, help="Path to taskonomy data")
    opt = parser.parse_args()

    export_path = os.path.join(opt.taskonomy_data_path, opt.model.lower()+'_semantic_skyboxes', 'semantic_skyboxes')

    if not os.path.exists(export_path):
        os.makedirs(export_path)

    scenegraph3d = {}
    scenegraph3d['graph'], scenegraph3d['panoramas'] = load_3DSceneGraph(opt.model, opt.sceneGraph_data_path)
    panos = scenegraph3d['panoramas']

    cat2ind, ind2cat, cat2ind_indoor = get_coco_dataset()
    ...
    for pano in panos:
        ...
        segm_pano = Image.new("RGB", (2048, 1024), (255, 255, 255))

        inst_class = segm_pano.copy()
        ...
        for ind, inst in enumerate(unique_instances):
            ...
            class_indoor = cat2ind_indoor[ind2cat[class_]]

            if inst not in class_instances[class_indoor]:
                class_instances[class_indoor].append(inst)
            class_instance = class_instances[class_indoor].index(inst) + 1

            r = class_indoor % 256
            g = (class_instance >> 8) % 256
            b = (class_instance & 255) % 256
            color_inst = np.array([r, g, b])
            print("cat:{} - class:{} - class_indoor:{} - inst:{} - class_inst:{} --> RGB:({}, {}, {})"
                  .format(ind2cat[class_], class_, class_indoor, inst, class_instance, r, g, b))
            for row in np.transpose(np.array(locs)):
                inst_pixs[int(row[1]), int(row[0])] = (color_inst[0], color_inst[1], color_inst[2])

        cubemap = e2c(np.array(inst_class))

        for i, face in enumerate(cubemap):
            idx = i
            if i == 4 or i == 0:
                idx = 4 - idx
            face_img = Image.fromarray(face)
            face_img.save(os.path.join(export_path, pano + '_semantic_skybox{}.png'.format(idx)))
