...
from generate_camera_poses import point_inside_mesh
import io_utils
...
def main():
    ...
    settings.MODEL_FILE = 'mesh_with_objs.ply'
    ...
    # Here points are defined as object locations
    points = get_points(model, bbox_corners)
    settings.NUM_POINTS = len(points)

    logger.info("Num cameras: {0}".format(len(camera_poses)))  
...
def get_points(model, bbox_corners):
    ...
    for obj_info in objs:
        p = obj_info[0]
        pos = [p[0], -p[2], p[1]]   # Don't forget to do this when using habitat positions!
        if pos[0] > x_range[-1] - 0.2 or pos[0] < x_range[0] + 0.2: continue
        if pos[1] > y_range[-1] - 0.2 or pos[1] < y_range[0] + 0.2: continue
        if pos[2] > z_range[-1] - 0.2 or pos[2] < z_range[0]: continue
        pos = Vector(pos)
        # if not point_inside_mesh(pos, model): continue
        loc = model.closest_point_on_mesh(pos)[1]
        points.append(loc)
    return points
...
def generate_point_correspondences(model, camera_poses, basepath):
    ...
    with Profiler("Generate fixated points", logger) as prf:
        ...
        logger.info("All objects: {0}".format(len(point_datas_and_next_points)))  
        point_datas_and_next_points = [p for p in point_datas_and_next_points if p != (None, None, None, None)]
        logger.info("Valid objects: {0}".format(len(point_datas_and_next_points)))  
...
def get_viable_point_and_corresponding_cameras(model, camera_locations, min_views=3, point_num=None, extra_info=None):
    ...
    # Generate point 
    candidate_point = points[point_num]
...
def evaluate_camera_views_of_point(camera_locations, candidate_point):
    ...
    for camera_uuid, camera_extrinsics in camera_locations.items():
        ...
        inside_frustum = is_point_inside_frustum(camera, candidate_point)
...
def filter_acceptable_cameras(camera_evaluations):
    ...
    for camera_uuid, camera_evaluation in camera_evaluations.items():
        distance = (Vector(camera_evaluation['point']) - Vector(camera_evaluation['camera_location'])).magnitude
        if distance < settings.MIN_DISTANCE_TO_OBJECT or distance > settings.MAX_DISTANCE_TO_OBJECT: continue
        if camera_evaluation['los'] and camera_evaluation['inside_frustum']: 
