...
case $task in
  ...
  depth_euclidean)
    ...
    ;;
  rgb)
    bash jobs/create_rgb_images_job.sh $model_path $args
    ;;
  reshading)
  ...
  segment25d)
    ...
    ;;
  semantic)
    bash jobs/create_semantic_images_clevr_job.sh $model_path $args
    ;;
  vanishing_points)
  ...
  points)
    ...
    ;;
  drop_objs)
    bash jobs/drop_google_objects_job.sh $model_path $args
    ;;
  add_objs)
